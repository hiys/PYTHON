...
def  send_msg(sender, receivers, content, subject):
  ...
  message['From']= Header(sender, 'utf-8')  #头部中的发送者Student
  message['To'] = Header(receivers[0], 'utf-8')  #头部中的接收者root
  #subject 头部中的主题信息
  ...
  #sender 发送者Student发送地址
  #接收者root, peri 地址列表receivers  
...
"""
  ...
  receivers = ['root@localhost', 'peri@localhost']  #接收者root, peri 地址>列表
  subject = 'Subject邮件SMTP标题'         #头部中的主题信息

  send_msg(sender, receivers, content, subject)
