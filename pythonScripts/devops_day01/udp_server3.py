...
#调用新客户端返回的ip地址 给客户端client_addr 循环 发送信息
count = 0
str_x = 'abc年月'
str_y = ''
while  count < 3:
  str_y += str_x
  server_sock.sendto(bytes(str_y, encoding = 'utf8'), client_addr)
  print(str_y,end ='\n\n')
  time.sleep(2)
  count += 1
server_sock.sendto(bytes('endover', encoding = 'utf8'), client_addr)
